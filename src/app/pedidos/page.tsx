...
import { Printer, Loader2, CheckCircle2, ArrowLeft } from 'lucide-react'
...
export default function PedidosPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage({ type: 'ok', text: 'Pedido registrado. Te contactamos por el email indicado.' })
    setNotes('')
    await refreshOrders()
  }
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        ...
        <div className="mb-8 flex items-start gap-4">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-violet-500/35 bg-violet-500/10 shadow-[0_0_24px_rgba(139,92,246,0.25)]">
            <Printer className="h-7 w-7 text-violet-300" aria-hidden />
          </div>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-white md:text-4xl">Imprenta</h1>
            <p className="mt-1 text-sm leading-relaxed text-white/55">
              Pedí impresión de figuritas, stickers o posters. Los pedidos llegan al panel de administración para
              coordinar producción y envío.
            </p>
          </div>
        </div>
        ...
        {!userReady ? (
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-8 text-center backdrop-blur-xl">
            <p className="text-white/70">Iniciá sesión para cargar un pedido de imprenta.</p>
            <Link
          ...
          <>
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Tamaño deseado, acabado mate/brillo, referencia a tu figurita del sitio, etc."
                />
              </div>
              ...
              >
                {submitting ? <Loader2 className="h-5 w-5 animate-spin" aria-hidden /> : <Printer className="h-5 w-5" aria-hidden />}
                Enviar pedido
              </button>
            </form>
            ...
            <div className="rounded-2xl border border-white/10 bg-[#0a0f1c]/60 p-6 backdrop-blur-xl">
              ...
              {loadingOrders ? (
                ...
                <p className="text-sm text-white/50">Todavía no tenés pedidos registrados.</p>
              ) : (
                <ul className="space-y-3">
                  {orders.map((o) => (
                    <li
                      key={o.id}
                      className="flex flex-col gap-1 rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"
                    >
                      <div>
